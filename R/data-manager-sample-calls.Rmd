...
headers <- dm_connect(email, password)
```

Get projects
```{r}
projects <- dm_get_projects()
print(projects)
```
...
```{r}
project_id <- 1
project_info <- dm_get_project(project_id)
print(project_info)
```

Get scenarios
```{r}
scenarios <- dm_get_scenarios()
print(scenarios)
```
...
```{r}
project_id <- 1
scenarios1 <- dm_get_scenarios_for_project(project_id)
print(scenarios1)
```
...
```{r}
project_id <- 1
variables <- dm_get_variables_for_project(project_id)
...
response <- dm_get_dataset_status(job_id, file_type)
