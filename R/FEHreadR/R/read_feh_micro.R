...
read_feh_cbk = function(codebookfile, fehfile, columns=NULL, obs_count=NA)
{
    ...
    while(obs_read<obs_count) {
        ...
        if(!is.null(columns)) {
            df = dplyr::select(df, tidyselect::all_of(columns))
        }
        ...
        if(obs_read == 0) {
            ...
            feh_df = dplyr::bind_rows(feh_df, df)
        }
...
#' @param read_frame a function that reads a frame in a header file (read_out_frame or read_in_frame)
#'
make_record_struct <- function(file, read_frame) {
...
read_out_frame = function(f)
{
    # Read binary data from file
    bin_data <- readBin(f, what = "raw", n = 44)
...
read_in_frame = function(f)
{
    # Read binary data from file
    unused = readBin(f, what="raw", size=1, n=2)
    frame  = readBin(f, what="int", size=4, n=4)
    unused = readBin(f, what="raw", size=1, n=2)
...
read_header = function(hdrfile)
{
    ...
    if( year == 0x20202020 ) {
        # Output header file
        sample_type = "output"
        read_frame = read_out_frame
        year = readBin(f, what="raw", n=6)
        year = as.integer(rawToChar(year))
    } else {
        # Input header file
        sample_type = "input"
        read_frame = read_in_frame
    }

    famrec <- make_record_struct(f, read_frame)
    perrec <- make_record_struct(f, read_frame)
...
#' @param suffix (optional) "even", "odd", or NULL
...
feh_read = function(fehdir, suffix="even", person=TRUE, columns=NULL, obs_count=NA)
{
    if( is.null(suffix) ) {
        ...
        stopifnot(suffix %in% c("even", "odd"))
        year_sfx = paste0('_', suffix)
    }
    type_sfx = ifelse(person,'person','family')

    hdrfile = paste0(fehdir, '/dynasipp_header', year_sfx, '.dat')
    datfile = paste0(fehdir, '/dynasipp_', type_sfx, year_sfx, '.dat')
